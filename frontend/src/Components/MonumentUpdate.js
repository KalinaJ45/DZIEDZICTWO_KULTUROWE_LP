...
function MonumentUpdate(props) {
	const navigate = useNavigate();
	const GlobalState = useContext(StateContext);

	const initialState = {
		nameValue: props.monumentData.name,
		inspireIdValue: props.monumentData.inspire_id,
		forestInspectorateValue: props.monumentData.administrator_forest_inspectorate,
		rdlpValue: props.monumentData.administrator_rdlp,
		categoryValue: props.monumentData.category,
		functionValue: props.monumentData.function,
		chronologyValue: props.monumentData.chronology,
		documentsValue: props.monumentData.document,
		descriptionValue: props.monumentData.description,
		leaseValue: props.monumentData.lease,
		availabilityValue: props.monumentData.availability,
		latitudeValue: props.monumentData.latitude,
		longitudeValue: props.monumentData.longitude,
		picture1Value: props.monumentData.picture1,
		picture2Value: props.monumentData.picture2,
		picture3Value: props.monumentData.picture3,
		mapInstance: null,
		markerPosition: {
			lat: props.monumentData.latitude,
			lng: props.monumentData.longitude,
		},
		uploadedPictures: [],
		sendRequest: 0,
		userProfile: {
		...
		if (state.sendRequest) {
			async function UpdateMonument() {
				...
				if (
					...
					formData.append("forest_inspectorate", state.forestInspectorateValue);
					formData.append("rdlp", state.rdlpValue);
				...
				try {
					const response = await Axios.patch(
						`http://localhost:8000/api/monuments/${props.monumentData.id}/update/`, formData);
	...
	function Picture1Display() {
		if (typeof state.picture1Value !== "string") {
			return (
	...
	function Picture2Display() {
		if (typeof state.picture2Value !== "string") {
			return (
	...
	function Picture3Display() {
		if (typeof state.picture3Value !== "string") {
			return (
				state.picture3Value ? (
		...
		<div style={{ width: "75%", marginLeft: "auto", marginRight: "auto", marginTop: "3rem", border: "5px solid black", padding: "3rem" }}>
			<form onSubmit={FormSubmit}>
				<Grid item container justifyContent="center">
					<Typography variant="h4">ZAKTUALIZUJ ZABYTEK</Typography>
				</Grid>
				...
				>
					...
					>
						UAKTUALNIJ FOTOGRAFIE (WYBIERZ 3 NOWE)
						<input
							type="file"
							multiple
				...
				>
					<Button
						...
						fullWidth
						type="submit"
						style={{ fontSize: "1.1rem", marginLeft: "1rem", "&:hover": { backgroundColor: "blue" } }}
				...
				<Button variant='contained' onClick={props.closeDialog}>ANULUJ</Button>
			</form>

			<Snackbar
				open={state.openSnack}
				message="Dane o zabytku zostały zaktualizowane!"
				anchorOrigin={{
					vertical: "bottom",
					horizontal: "center",
				}}
